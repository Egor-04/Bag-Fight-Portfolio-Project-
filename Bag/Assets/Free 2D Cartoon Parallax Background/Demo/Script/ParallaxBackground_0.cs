...
{
    [SerializeField] private bool Camera_Move;
    [SerializeField] private float Camera_MoveSpeed = 1.5f;
    ...
    private void Update(){
        ...
        if (Camera_Move)
        {
            Vector3 camPos = Vector3.right * Time.deltaTime * Camera_MoveSpeed;
            _camera.position += new Vector3(camPos.x, 0f, camPos.z);

            EventManager.Instance.PlayerMoveInvoke(true);
        }
        else
        {
            EventManager.Instance.PlayerMoveInvoke(false);
        }
